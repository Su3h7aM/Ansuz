...
// is_default_color checks if a color is the default terminal color
is_default_color :: proc(color: TerminalColor) -> bool {
...
generate_style_sequence :: proc(fg, bg: TerminalColor, styles: StyleFlags) -> string {
	...
	if is_default_color(fg) && is_default_color(bg) && card(styles) == 0 {
	...
	if !is_default_color(fg) {
		...
		builder = append_fg_color(builder, fg)
		first = false
	}
	...
	if !is_default_color(bg) {
		...
		builder = append_bg_color(builder, bg)
		first = false
	}
...
// append_fg_color appends the foreground color sequence to builder
append_fg_color :: proc(builder: string, color: TerminalColor) -> string {
	switch c in color {
	case Ansi:
...
// append_bg_color appends the background color sequence to builder
append_bg_color :: proc(builder: string, color: TerminalColor) -> string {
	switch c in color {
	case Ansi:
...
to_ansi :: proc(s: Style) -> string {
	return generate_style_sequence(s.fg, s.bg, s.flags)
}
