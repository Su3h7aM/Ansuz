...
@(test)
test_wait_result_enum :: proc(t: ^testing.T) {
    testing.expect(t, WaitResult.None == .None, "None should be valid")
    testing.expect(t, WaitResult.Input == .Input, "Input should be valid")
    testing.expect(t, WaitResult.Resize == .Resize, "Resize should be valid")
    testing.expect(t, WaitResult.Error == .Error, "Error should be valid")
}
...
test_function_pointers_exist :: proc(t: ^testing.T) {
    ...
    testing.expect(t, ptr_wait != nil, "wait_for_event should exist")
}
...
@(test)
test_wait_for_event_function_type :: proc(t: ^testing.T) {
    fn_type: proc(int, int, i32) -> (WaitResult, int, int)
    fn_type = wait_for_event
    testing.expect(t, fn_type != nil, "wait_for_event should exist")
}
