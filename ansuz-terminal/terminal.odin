...
init_terminal :: proc() -> TerminalError {
	if _global_terminal_state.is_initialized {
		return .None
	}
	...
	result := posix.tcgetattr(stdin_fd, &_global_terminal_state.original_termios)
	if result != .OK {
		return .FailedToGetAttributes
	}

	_global_terminal_state.is_initialized = true
...
enter_raw_mode :: proc() -> TerminalError {
	...
	if _global_terminal_state.is_raw_mode {
		return .None
	}

	raw := _global_terminal_state.original_termios
	...
	_global_terminal_state.is_raw_mode = true
	return .None
}
...
leave_raw_mode :: proc() -> TerminalError {
	...
	result := posix.tcsetattr(stdin_fd, .TCSAFLUSH, &_global_terminal_state.original_termios)
	if result != .OK {
		return .FailedToSetAttributes
	}

	_global_terminal_state.is_raw_mode = false
	return .None
}
...
// WaitResult indicates what caused wait operations to return
WaitResult :: enum {
...
//
// Parameters:
//   current_width:  Current terminal width for resize detection
//   current_height: Current terminal height for resize detection
//   timeout_ms:     Maximum time to wait (-1 = infinite, 0 = immediate, >0 = milliseconds)
//
// Returns: (WaitResult, new_width, new_height)
//   On .Resize: new_width/new_height contain updated dimensions
//   Otherwise:  dimensions remain unchanged
wait_for_event :: proc(current_width, current_height: int, timeout_ms: i32 = -1) -> (result: WaitResult, new_width: int, new_height: int) {
	...
	for {
		...
		size_width, size_height, _ := get_terminal_size()
		if size_width != current_width || size_height != current_height {
