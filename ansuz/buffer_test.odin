...
test_buffer_init_invalid_dimensions :: proc(t: ^testing.T) {
    ...
    testing.expect(t, err4 == .InvalidDimensions, "Negative height should return InvalidDimensions")
}
...
test_buffer_clear :: proc(t: ^testing.T) {
    ...
    buffer.cells[0].fg_color = .Red
    
    clear_buffer(&buffer)
    
    testing.expect(t, buffer.cells[0].rune == ' ', "Cell should be space after clear")
    testing.expect(t, buffer.cells[0].fg_color == .Default, "Cell color should be default after clear")
    testing.expect(t, buffer.cells[0].bg_color == .Default, "Cell bg should be default after clear")
...
test_buffer_get_cell_safe :: proc(t: ^testing.T) {
    ...
    testing.expect(t, cell.rune == ' ', "Cell should have default rune")
    testing.expect(t, cell.fg_color == .Default, "Cell should have default fg")
    testing.expect(t, cell.bg_color == .Default, "Cell should have default bg")
    
    cell_out := get_cell_safe(&buffer, 100, 100)
    testing.expect(t, cell.rune == ' ', "Out of bounds cell should return default")
    testing.expect(t, cell.fg_color == .Default, "Out of bounds cell should have default fg")
...
test_buffer_write_string :: proc(t: ^testing.T) {
    ...
    chars := write_string(&buffer, 0, 0, "Hello, World!", .Red, .Blue, {})
    ...
    testing.expect(t, cell.fg_color == .Red, "First char should have red fg")
...
test_buffer_write_string_bounds :: proc(t: ^testing.T) {
    ...
    chars := write_string(&buffer, 0, 0, "0123456789", .Red, .Blue, {})
    testing.expect(t, chars == 10, "Should write all 10 chars")
    
    chars = write_string(&buffer, 0, 0, "01234567890", .Red, .Blue, {})
    testing.expect(t, chars == 10, "Should truncate to buffer width")
    
    chars = write_string(&buffer, 5, 0, "0123456789", .Red, .Blue, {})
    testing.expect(t, chars == 5, "Should write only 5 chars from x=5")
    
    chars = write_string(&buffer, -5, 0, "ABC", .Red, .Blue, {})
...
test_buffer_write_string_out_of_bounds_y :: proc(t: ^testing.T) {
    ...
    chars := write_string(&buffer, 0, 5, "Test", .Red, .Blue, {})
    testing.expect(t, chars == 0, "Should write 0 chars for y >= height")
    
    chars = write_string(&buffer, 0, 10, "Test", .Red, .Blue, {})
    testing.expect(t, chars == 0, "Should write 0 chars for y > height")
    
    chars = write_string(&buffer, 0, -1, "Test", .Red, .Blue, {})
...
test_buffer_write_string_empty :: proc(t: ^testing.T) {
    ...
    chars := write_string(&buffer, 0, 0, "", .Red, .Blue, {})
...
test_buffer_fill_rect :: proc(t: ^testing.T) {
    ...
    fill_rect(&buffer, 5, 5, 10, 5, 'X', .Red, .Blue, {.Bold})
    ...
    for y in 5 ..< 10 {
        for x in 5 ..< 15 {
            ...
            testing.expect(t, cell.fg_color == .Red, "Cell should have red fg")
            testing.expect(t, cell.bg_color == .Blue, "Cell should have blue bg")
...
test_buffer_draw_box :: proc(t: ^testing.T) {
    ...
    draw_box(&buffer, 5, 3, 10, 5, .Red, .Blue, {})
...
test_buffer_draw_box_too_small :: proc(t: ^testing.T) {
    ...
    draw_box(&buffer, 5, 3, 1, 5, .Red, .Blue, {})
    draw_box(&buffer, 5, 3, 5, 1, .Red, .Blue, {})
...
test_buffer_resize :: proc(t: ^testing.T) {
    ...
    testing.expect(t, cell.fg_color == .Red, "Color should be preserved")
...
test_buffer_resize_smaller :: proc(t: ^testing.T) {
    ...
    for y in 0 ..< 5 {
        for x in 0 ..< 5 {
            set_cell(&buffer, x, y, 'A', .Red, .Blue, {})
        }
    }
...
test_buffer_multiple_styles :: proc(t: ^testing.T) {
    ...
    set_cell(&buffer, 5, 5, 'X', .Red, .Blue, styles)
...
test_buffer_large_dimensions :: proc(t: ^testing.T) {
    ...
    testing.expect(t, len(buffer.cells) == 200 * 100, "Large buffer should have correct cell count")
    
    set_cell(&buffer, 199, 99, 'X', .Red, .Blue, {})
...
test_buffer_write_multiple_lines :: proc(t: ^testing.T) {
    ...
    write_string(&buffer, 0, 0, "Line0", .Red, .Blue, {})
    write_string(&buffer, 0, 1, "Line1", .Red, .Blue, {})
    write_string(&buffer, 0, 2, "Line2", .Red, .Blue, {})
