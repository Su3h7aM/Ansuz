...
Element :: struct {
    using layout: al.LayoutConfig,
    style:        ac.Style,
    box_style:    Maybe(ab.BoxStyle),
...
element_default :: proc() -> Element {
    return Element {
        layout = al.DEFAULT_LAYOUT_CONFIG,
...
_process_element_start :: proc(ctx: ^Context, el: Element, is_container: bool) {
    ...
    if is_container {
        ...
        node_idx := al.begin_container(l_ctx, config)
...
@(private)
_element_to_render_cmd :: proc(el: Element) -> al.RenderCommand {
    if txt, ok := el.content.?; ok {
        return al.RenderCommand{type = .Text, text = txt, style = el.style}
    }

    if bs, ok := el.box_style.?; ok {
        return al.RenderCommand{type = .Box, style = el.style, box_style = bs}
    }

    if char, ok := el.fill_char.?; ok {
        return al.RenderCommand{type = .Rect, style = el.style, char = char}
    }

    return al.RenderCommand{type = .None}
}
...
interact :: proc(ctx: ^Context, id: u64, rect: ab.Rect) -> Interaction {
...
widget_button :: proc(ctx: ^Context, lbl: string) -> bool {
    ...
    interaction := interact(ctx, elem_id, ab.Rect{})
...
widget_select :: proc(ctx: ^Context, lbl: string, options: []string, selected_idx: ^int, is_open: ^bool) -> bool {
	...
	if is_open^ {
		...
		opt_style := ac.Style{fg = ac.Ansi.White, bg = ac.Ansi.Default, flags = {}}
		if is_selected {
			opt_style = ac.Style{fg = ac.Ansi.Black, bg = ac.Ansi.BrightCyan, flags = {.Bold}}
		}
