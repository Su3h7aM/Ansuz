...
Cell :: struct {
	...
	fg:    ac.TerminalColor, // Foreground (text) color
	bg:    ac.TerminalColor, // Background color
	style: ac.StyleFlags,    // Text attributes (bold, underline, etc.)
}
...
// is_clipped checks if a point is outside the current clip rect
is_clipped :: proc(buffer: ^FrameBuffer, x, y: int) -> bool {
...
clear_buffer :: proc(buffer: ^FrameBuffer) {
	for &cell in buffer.cells {
		cell.rune = ' '
		cell.fg = ac.Ansi.Default
		cell.bg = ac.Ansi.Default
		cell.style = {}
	}
}
...
write_string :: proc(
	...
	for r in text {
		...
		if current_x >= 0 {
			// Check clipping
			if !is_clipped(buffer, current_x, y) {
...
draw_box :: proc(
	...
	if x >= 0 && x < buffer.width && y >= 0 && y < buffer.height && !is_clipped(buffer, x, y) {
		index := y * buffer.width + x
	...
	if x >= 0 &&
	   ...
	   !is_clipped(buffer, x, y + height - 1) {
	...
	if x + width - 1 >= 0 &&
	   ...
	   y + height - 1 >= 0 &&
	   y + height - 1 < buffer.height &&
	   !is_clipped(buffer, x + width - 1, y + height - 1) {
	...
	for dx in 1 ..< width - 1 {
		...
		if px >= 0 &&
		   ...
		   y + height - 1 >= 0 &&
		   y + height - 1 < buffer.height &&
		   !is_clipped(buffer, px, y + height - 1) {
	...
	for dy in 1 ..< height - 1 {
		...
		if x + width - 1 >= 0 &&
		   ...
		   py >= 0 &&
		   py < buffer.height &&
		   !is_clipped(buffer, x + width - 1, py) {
...
render_to_string :: proc(
	...
	current_style := ac.default_style()
	...
	for y in 0 ..< render_h {
		...
		for x in 0 ..< render_w {
			...
			if cell == nil {
				...
				if current_style != ac.default_style() {
					style_seq := ac.to_ansi(ac.default_style())
				...
				if new_style != current_style {
					style_seq := ac.to_ansi(new_style)
					strings.write_string(builder, style_seq)
	...
	if needs_style_reset {
		strings.write_string(builder, ac.reset_style())
	}
